...
If you open the dataset in the data viewer of RStudio you will see that each column has a variable name and underneath a longer and more meaninful *variable label* that tells you what each variable means. 

You also want to understand what the csew object actually is. You can do that using the *class()* function:
...
So this object has different compartments. The first one is called label and it provides a description of what the variable sex measures. This is what you saw in the RStudio data viewer earlier. The second compartment explains the original format in which it was. The third one identifies the class of the vector. Whereas the final one *labels* provides the labels that allows us to identify what the meaning of 1 and 2 are in this context.
...
##Recoding data in R and more fun with dplyr

Often you will be place in a situation where you need to do some transformations to the variables that you are working with. In this section we are going to go back to the csew tibble. But before we get into recoding, and so that you appreciate the differences when recoding numeric and categorical variables, first we are going to recode the labelled vectors in this tibble into factors. If you were dealing with something like SPSS most likely you would need to do a lot of point and clicking to get this done, and it would take you quite a bit of time. One of the advantages of using code is that we can automatise things, which means that it is the computer, not us, who does the more tedious work. 

In order to mutate all labelled vectors into factors we can use the *mutate_if()* dplyr function using the code below:

```{r}
#We want to store the results, so we will create new tibble where the categorical variables are
#encoded as factors. To do this we will apply the mutate_if function. First we ask for the columns that happened to be labelled vectors and then we ask R to coerce these into factors.
#If we want to check our results we can now see the class of the columns in the new tibble. You will indeed see that we are now dealing with factors. Notice, all we needed was one single line of code. 
csew_f <- csew %>% mutate_if(is.labelled, as_factor)
lapply(csew_f, class)
summary(csew_f)

```

You may have noticed a funny symbol in the code above: *%>%*. This is the so-called **piping operator**. It is a convenient operator in R programming that can be interpreted as "and then". So in the previous code first we said to R create a new object called csew_f in which you are going to put the contents of csew "and then" mutate the columns into factors if they were labelled. This very convenient operator makes writing code a little bit easier. We will see more examples as we go along.

We have talked already about attributes in an object. You can think of these as compartments inside your box. And if you know the name of the compartment you can extract the information on it easily. 
