...
RStudio allows you to customise the way it looks. Working with white backgrounds is not generally a good idea if you care about your eyesight. I'm talking from experience. If you don't want to end up with dry eyes not only it is good you follow the 20-20-20 rule (every 20 minutes look for 20 seconds to an object located 20 feet away from you), but it may also be a good idea to use more eye friendly screen displays. 

Click in the *Tools* menu and select *Global options*. This will open up a pop up window with various options. Select *appearance*. In this section you can change the font type and size, but also the kind of theme background that R will use in the various windows. I use the *Tomorrow Night Bright* theme, but you may prefer a different one. You can preview them and then click apply to select the one you like. This will do nothing other to any of your results or analysis. This is just something you may want to do in order to make things look better and healthier for your.
...
We will specify, however, a different destination for the file. This requires that you understand how to store things inside your computer. Do you know what a pathfile or a directory is? You may want to have a quick look at the [Wiki article](https://en.wikipedia.org/wiki/Path_(computing)) on it. There is an easy way to figure out what is the pathfile for the project you are currently working on. You can use the getwd() function to do this:

```{r, eval=FALSE}
getwd()
```
If you look at the Files window in the bottom right corner that's the folder where you are working from. My pathfile for my project is this:

"C:/Users/msrtsjm2/Dropbox/1_Teaching/1 Manchester courses/20452 Modelling Criminological Data/R4Crim"

But for yours it will be something different.Use the pathfile you obtain to place the resulting audio file in the right directory. So, for example, if you are using a computer cluster and want to place it in a folder called LAWS20452 you would use the code below:


```{r, eval=FALSE}
tts_ITRI(my_text, destfile="P:/LAWS20452/i_hate.wav")
```
...
```{r, eval=FALSE}
library("tuneR")
play("P:LAWS20452/i_hate.wav")
```

If you are using a MAC you may be getting an error message when doing this, if this is the case. Please use the following fix (but notice that you will need to alter the pathfile):

```{r, eval = FALSE}
setWavPlayer('/usr/bin/afplay')
play(shQuote("/Users/reka/Dropbox (The University of Manchester)/31152_60142 GIS and Crime Mapping/2018_labs/i_hate.wav"))
